...
function showView(name) {
  const valid = ["home", "days", "gallery", "timeline", "reasons", "dreams", "nazar", "punish", "universe", "openwhen", "angry", "secret"];
...
function openLetter(day, letter) {
  ...
  // Stop any Day 26 premium-player audio left over from a previously opened letter.
  ...
  // Day 26 gets the premium Apple-style voice player (real audio, waveform,
  // progress, duration) instead of the plain "Play Voice Note" pill used by
  // every other letter. Every other day is completely untouched.
  const premiumWrap = $("#letterPremiumAudio");
  if (day === 26) {
    newVoiceBtn.hidden = true;
    premiumWrap.hidden = false;
    dayPremiumAudioCleanup = renderDay26VoicePlayer(premiumWrap, letter.voice);
  } else {
...
// Day 26's premium voice player. Reuses the same safe pattern as the Open
// When Letters player: the <audio> element plays back completely natively
// (no Web Audio rerouting, so it's always genuinely audible), and the
// waveform is drawn from a separate, disconnected offline decode that can
// never affect playback even if it fails.
function renderDay26VoicePlayer(container, audioSrc) {
  container.innerHTML = `
    <p class="openwhen-pretext" id="day26Pretext">Close your eyes...<br>Pretend I'm sitting right beside you.<br>Now just listen to me. ❤️</p>
    <div class="voice-player glass">
      <button class="voice-play-btn" id="day26PlayBtn" aria-label="Play voice note">▶</button>
      <div class="voice-player-main">
        <div class="voice-waveform"><canvas id="day26WaveformCanvas"></canvas></div>
        <div class="voice-progress-track"><div class="voice-progress-fill" id="day26ProgressFill"></div></div>
        <div class="voice-time"><span id="day26CurrentTime">0:00</span> / <span id="day26Duration">0:00</span></div>
      </div>
    </div>
    <audio id="day26Audio" src="${audioSrc}" preload="metadata" playsinline></audio>
    <p class="openwhen-footer-line" id="day26AfterText" hidden>If you still miss me...<br>press Play one more time. ❤️</p>
  `;

  const audio = $("#day26Audio", container);
  audio.volume = 1;
  audio.muted = false;

  const playBtn = $("#day26PlayBtn", container);
  const canvas = $("#day26WaveformCanvas", container);
  const fill = $("#day26ProgressFill", container);
  const curEl = $("#day26CurrentTime", container);
  const durEl = $("#day26Duration", container);
  const pretext = $("#day26Pretext", container);
  const afterText = $("#day26AfterText", container);
