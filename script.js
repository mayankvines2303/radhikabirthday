...
function showView(name) {
  const valid = ["home", "days", "gallery", "timeline", "reasons", "dreams", "nazar", "punish", "universe", "openwhen", "angry", "kilometer", "okay", "garden", "secret"];
...
function gardenDefaultState() {
  return { planted: false, plantedAt: null, totalWaters: 0, waterTarget: 0, lastWaterDate: null, watersToday: 0, bloomed: false, wish: null };
}
...
function renderGardenUI() {
  ...
  if (state.bloomed) {
    ...
    const tripBtn = $("#gardenWishTripBtn");
    const stayBtn = $("#gardenWishStayBtn");
    const miniMayank = $("#gardenMiniMayank");
    const endingText = $("#gardenEndingText");
    const chosenLabel = $("#gardenChosenWish");

    if (state.wish) {
      tripBtn.disabled = true; stayBtn.disabled = true;
      tripBtn.classList.toggle("garden-wish-selected", state.wish === "trip");
      stayBtn.classList.toggle("garden-wish-selected", state.wish === "stay");
      chosenLabel.textContent = "You wished for: " + (state.wish === "trip" ? "A Trip Together Soon" : "Stay With Me For A Whole Day & Night");
      miniMayank.hidden = false;
      requestAnimationFrame(() => miniMayank.classList.add("in"));
      endingText.hidden = false;
      requestAnimationFrame(() => endingText.classList.add("in"));
    } else {
      tripBtn.disabled = false; stayBtn.disabled = false;
      tripBtn.classList.remove("garden-wish-selected");
      stayBtn.classList.remove("garden-wish-selected");
      miniMayank.hidden = true; miniMayank.classList.remove("in");
      endingText.hidden = true; endingText.classList.remove("in");
    }
    return;
  }
...
function onGardenChooseWish(choice) {
  const state = gardenLoadState();
  if (!state.bloomed || state.wish) return; // only one wish, only after blooming
  state.wish = choice;
  gardenSaveState(state);
  renderGardenUI();
}
...
function initGardenPage() {
  ...
  $("#gardenWishTripBtn").addEventListener("click", () => onGardenChooseWish("trip"));
  $("#gardenWishStayBtn").addEventListener("click", () => onGardenChooseWish("stay"));
  renderGardenUI();
}
