...
function showView(name) {
  const valid = ["home", "days", "gallery", "timeline", "reasons", "dreams", "nazar", "punish", "universe", "openwhen", "angry", "kilometer", "okay", "garden", "os", "star", "cosmos", "museum", "eyes", "sim", "decision", "video13", "coin", "timemachine", "lifedream", "secret"];
  ...
  // Another one-shot cinematic experience — same fresh-start-every-visit,
  // fully-stop-on-navigate-away pattern as the Time Machine above.
  if (name === "lifedream" && typeof resetLifeDreamFlow === "function") resetLifeDreamFlow();
  else if (typeof stopLifeDreamFlow === "function") stopLifeDreamFlow();
...
/* ============================================================================
   THE LIFE I DREAM WITH YOU
   ------------------------------------------------------------------------
   A warm cinematic scene-deck: one persistent canvas + crossfading gradient
   backdrop, with thirteen small living "scenes" stitched together. Reuses
   tmLerp()/tmLerpColor() from the Time Machine module above (plain math
   helpers, no reason to duplicate them). Same reset/stop-on-navigate-away
   shape as every other one-shot experience on this site.
   ============================================================================ */

const LD_LAYER_IDS = [
  "ldIntro", "ldMorning", "ldDrive", "ldGrocery", "ldCooking", "ldRainy",
  "ldTravel", "ldFestivals", "ldStorm", "ldOldTogether", "ldPromises",
  "ldFinale", "ldNotebook",
];
// Progress dots only cover the "living" scenes — not the silent intro or
// the secret notebook epilogue.
const LD_DOT_RANGE = [1, 11];

const LD_BG_THEMES = [
  "radial-gradient(ellipse at 50% 30%, #241a2e, #0c0806 75%)",                 // intro — quiet violet dark
  "radial-gradient(ellipse at 50% 20%, #6b4a2a, #2a180f 70%)",                 // morning — sunrise gold
  "linear-gradient(180deg, #3b4a63 0%, #6c5a52 55%, #a8664a 100%)",            // drive — dusk road
  "radial-gradient(ellipse at 50% 10%, #4a3420, #1c130a 75%)",                 // grocery — warm cream light
  "radial-gradient(ellipse at 50% 60%, #4a2414, #170d08 75%)",                 // cooking — stove-warm orange
  "radial-gradient(ellipse at 50% 70%, #223244, #0a0e14 75%)",                 // rainy — deep blue grey
  "radial-gradient(ellipse at 50% 20%, #1f4a4a, #0a1616 75%)",                 // travel — teal horizon
  "radial-gradient(ellipse at 50% 20%, #5a1f3a, #1a0a12 75%)",                 // festivals — magenta glow
  "radial-gradient(ellipse at 50% 40%, #2a2f3a, #0a0c10 75%)",                 // storm — desaturated grey
  "radial-gradient(ellipse at 50% 30%, #4a3018, #170f08 75%)",                 // old together — sepia amber
  "radial-gradient(ellipse at 50% 20%, #2e1f4a, #0e0a1a 75%)",                 // promises — deep violet gold
  "radial-gradient(ellipse at 50% 15%, #10142a, #04050c 80%)",                 // finale — midnight navy
  "radial-gradient(ellipse at 50% 30%, #241a2e, #0c0806 75%)",                 // notebook — soft, quiet
];

const LD_AMBIENT_MODES = [
  "dust", "dust", "none", "sparkle", "none", "rain",
  "dust", "sparkle", "rain", "dust", "sparkle", "lantern", "dust",
];

const LD_INTRO_LINES = [
  "Can I show you something?",
  "Not our past…",
  "But the life I quietly dream of with you.",
];

const LD_MORNING_ITEMS = [
  { icon: "☕", x: "12%", y: "12%" }, { icon: "☕", x: "26%", y: "30%" },
  { icon: "🪥", x: "58%", y: "8%" }, { icon: "🪥", x: "67%", y: "22%" },
  { icon: "🩴", x: "8%", y: "72%" }, { icon: "🩴", x: "20%", y: "80%" },
  { icon: "🧥", x: "80%", y: "50%" }, { icon: "☀️", x: "46%", y: "4%" },
];

const LD_GROCERY_ITEMS = [
  { icon: "🍫", label: "Chocolate" }, { icon: "🍦", label: "Ice Cream" },
  { icon: "💐", label: "Flowers" }, { icon: "☕", label: "Coffee" },
  { icon: "🍜", label: "Maggi" }, { icon: "🍪", label: "Cookies" },
];

const LD_COOKING_INGREDIENTS = [
  { icon: "🍅", label: "Tomato" }, { icon: "🧀", label: "Cheese" },
  { icon: "🍫", label: "Chocolate" }, { icon: "🌶️", label: "Chili" },
  { icon: "🍚", label: "Rice" }, { icon: "🥚", label: "Egg" },
];
const LD_COOKING_COMBOS = {
  "Cheese,Tomato": "A questionable pasta. We'll call it a win anyway. 🍝",
  "Chocolate,Tomato": "Absolutely not. We're never doing that again. 😂",
  "Cheese,Rice": "Somehow… actually really good? 🍚",
  "Chili,Egg": "You cried. Not from emotion. 🌶️",
  "Chocolate,Egg": "A cake happened. Mostly by accident. 🍰",
  "Chili,Rice": "Perfectly, deliciously, too spicy. 🔥",
  "Cheese,Chocolate": "We do not talk about this one. 🙈",
  "Chili,Tomato": "Somewhere between soup and a dare. 🌶️",
};
const LD_COOKING_FALLBACK = "It's… edible. We'll take it. 🍲";

const LD_DRIVE_LINES = [
  "I don't really care where life takes us…",
  "…as long as we're going there together.",
];

const LD_TRAVEL_ICONS = [
  { icon: "✈️", top: "16%", left: "10%" }, { icon: "🚆", top: "58%", left: "62%" },
  { icon: "🚗", top: "76%", left: "22%" }, { icon: "🎫", top: "34%", left: "78%" },
  { icon: "🗺️", top: "8%", left: "48%" }, { icon: "🛂", top: "62%", left: "6%" },
];
const LD_TRAVEL_LINES = [
  "Let's collect places…",
  "…and even more importantly, let's collect moments.",
];

const LD_FESTIVAL_CHIPS = [
  { icon: "🪔", label: "Diwali evenings" }, { icon: "🎨", label: "Holi mornings" },
  { icon: "🎂", label: "Your birthday, every year" }, { icon: "❄️", label: "Winter nights, wrapped up" },
  { icon: "🎄", label: "Quiet family moments" }, { icon: "🎆", label: "New Year, just us" },
];

const LD_STORM_LINES = [
  "I don't promise a life without storms.",
  "I only promise that I won't stop choosing us.",
];

const LD_FINALE_LINES = [
  "People often ask what my biggest dream is.",
  "It was never a place.",
  "It was never success.",
  "It has always been you.",
];

// One hundred small, specific promises — deliberately varied in shape and
// scale so the wall doesn't read as a templated list.
const LD_PROMISES = [
  "I'll always save you the last bite.",
  "I'll learn your coffee order and never get it wrong.",
  "I'll hold the umbrella so it covers you more than me.",
  "I'll let you pick the movie, even when I don't want to.",
  "I'll warm up your side of the bed first.",
  "I'll never go to sleep genuinely angry.",
  "I'll remember the small things you mention once.",
  "I'll dance with you in the kitchen for no reason.",
  "I'll always answer when you call twice in a row.",
  "I'll tell you the truth, even the inconvenient kind.",
  "I'll cheer loudest for your smallest wins.",
  "I'll never make you feel silly for caring so much.",
  "I'll keep choosing you, on the boring days too.",
  "I'll learn to like the things you love.",
  "I'll never let you feel alone in a crowded room.",
  "I'll build a home with you, not just a house.",
  "I'll fight for us, not against you.",
  "I'll say sorry first, even when it's hard.",
  "I'll celebrate your growth, even when it outpaces mine.",
  "I'll never compare you to anyone else.",
  "I'll protect your peace like it's my own.",
  "I'll show up, especially when it's inconvenient.",
  "I'll let you be the reason I laugh mid-argument.",
  "I'll keep your secrets safer than my own.",
  "I'll never stop being curious about you.",
  "I'll hold your hand in places that scare you.",
  "I'll make you tea without being asked.",
  "I'll remember your parents' birthdays too.",
  "I'll never let a fight decide our future.",
  "I'll be your soft place to land.",
  "I'll cry at your favourite movie with you.",
  "I'll drive the long way if you want to talk.",
  "I'll never make your dreams feel smaller than mine.",
  "I'll keep loving you after the butterflies settle.",
  "I'll pack your favourite snacks for the road.",
  "I'll let you win sometimes. Sometimes.",
  "I'll never go quiet when it matters most.",
  "I'll notice when you're pretending to be okay.",
  "I'll take the blanket you keep stealing, and let you have it anyway.",
  "I'll wait for you, at the airport and everywhere else.",
  "I'll always introduce you as the best part of my day.",
  "I'll keep every playlist you've ever made me.",
  "I'll never let distance make us strangers.",
  "I'll learn to fight fair, and forgive faster.",
  "I'll remember how you take your eggs.",
  "I'll never let you doubt that you're chosen.",
  "I'll keep showing up for your family too.",
  "I'll let our home be loud with laughter.",
  "I'll never stop writing you letters, even short ones.",
  "I'll hold space for your bad days without trying to fix them.",
  "I'll be proud of you loudly, and in private.",
  "I'll keep dreaming in 'we', not just 'me'.",
  "I'll never make you ask twice for reassurance.",
  "I'll learn your love language and speak it fluently.",
  "I'll keep the porch light on for you, literally and otherwise.",
  "I'll remember this exact promise wall, years from now.",
  "I'll never let routine erase romance.",
  "I'll surprise you on ordinary Tuesdays.",
  "I'll keep choosing patience over pride.",
  "I'll always make room for your opinions.",
  "I'll never let you feel like a burden for needing me.",
  "I'll learn to sit in silence with you comfortably.",
  "I'll keep your coffee mug filled before mine.",
  "I'll never stop trying to make you proud.",
  "I'll remember our first everything.",
  "I'll hold onto us through every version of ourselves.",
  "I'll never let ego win over 'us'.",
  "I'll keep making you laugh until we're both wheezing.",
  "I'll be gentle with your heart, always.",
  "I'll never take your trust for granted.",
  "I'll keep our inside jokes alive for decades.",
  "I'll learn something new just because you love it.",
  "I'll never let you carry everything alone.",
  "I'll keep choosing honesty over comfort.",
  "I'll remember to say 'thank you' even for the small things.",
  "I'll never let you feel unseen.",
  "I'll keep planning trips we may never take, just for the joy of it.",
  "I'll hold your hand during every scary movie.",
  "I'll never stop being your biggest fan.",
  "I'll learn to apologize without 'but'.",
  "I'll keep every voice note you've ever sent me.",
  "I'll never let you think you're too much.",
  "I'll show you off without being asked to.",
  "I'll keep choosing 'us' over 'being right'.",
  "I'll never let your birthday pass unnoticed.",
  "I'll remember how you like your bed made.",
  "I'll keep growing, so I never stop deserving you.",
  "I'll never let you feel like you're chasing me.",
  "I'll hold you a little longer than necessary.",
  "I'll keep saying 'I love you' until it's not enough words for it.",
  "I'll never let silence replace communication.",
  "I'll remember this promise, especially on hard days.",
  "I'll keep choosing you tomorrow, and the day after.",
  "I'll never stop being amazed that it's you.",
  "I'll hold our story with both hands.",
  "I'll keep making memories on ordinary days.",
  "I'll never let 'busy' become an excuse.",
  "I'll remember that love is a daily choice.",
  "I'll keep choosing this, over and over, for as long as I live.",
  "I'll love you exactly as you are, and cheer you into who you're becoming.",
];
...
const ldState = {
  ...
  ambientCount: 0,
  particles: [],
  bgActive: "a",        // which crossfade layer is currently visible
  cartCount: 0,
  cookingPicked: [],
  cookingCombosSeen: new Set(),
  yearsValue: 0,
  promisesOrder: [],
  promisesRevealed: 0,
  driveOffset: 0,
...
/* ---------------------------------------------------------------------------
   TIMERS
   --------------------------------------------------------------------------- */
function ldAfter(ms, fn) {
  const t = setTimeout(fn, ms);
  ldState.timers.push(t);
  return t;
}
function ldClearTimers() {
  ldState.timers.forEach((t) => clearTimeout(t));
  ldState.timers = [];
  ldState.cancelFns.forEach((fn) => fn && fn());
  ldState.cancelFns = [];
}

/* ---------------------------------------------------------------------------
   SYNTHESISED AMBIENT SOUND — its own lazy AudioContext, same pattern as
   the Time Machine's, so the two pages never fight over one audio graph.
   --------------------------------------------------------------------------- */
let _ldAudioCtx = null;
let _ldMasterGain = null;
let _ldPadNodes = null;
function ldGetAudioCtx() {
  if (!_ldAudioCtx) {
    _ldAudioCtx = new (window.AudioContext || window.webkitAudioContext)();
    _ldMasterGain = _ldAudioCtx.createGain();
    _ldMasterGain.gain.value = ldState.muted ? 0 : 0.14;
    _ldMasterGain.connect(_ldAudioCtx.destination);
  }
  if (_ldAudioCtx.state === "suspended") _ldAudioCtx.resume();
  return _ldAudioCtx;
}
function ldStartPad() {
  try {
    const ctx = ldGetAudioCtx();
    const freqs = [130.8, 164.8, 196.0]; // a soft, warm C major triad
    const gain = ctx.createGain();
    gain.gain.value = 0;
    gain.connect(_ldMasterGain);
    ...
    gain.gain.linearRampToValueAtTime(0.32, ctx.currentTime + 3);
    _ldPadNodes = { oscs, gain };
  } catch (e) { _ldPadNodes = null; }
}
function ldStopPad() {
  if (!_ldPadNodes) return;
  try {
    const ctx = ldGetAudioCtx();
    _ldPadNodes.gain.gain.linearRampToValueAtTime(0, ctx.currentTime + 0.8);
    _ldPadNodes.oscs.forEach((o) => o.stop(ctx.currentTime + 0.85));
  } catch (e) { /* noop */ }
  _ldPadNodes = null;
}
function ldPlayChime(notes) {
  try {
    const ctx = ldGetAudioCtx();
    notes.forEach((freq, i) => {
      const osc = ctx.createOscillator();
      ...
      osc.connect(gain).connect(_ldMasterGain);
      osc.start(start);
      osc.stop(start + 1.2);
    });
  } catch (e) { /* noop */ }
}
function ldPlayPop() { ldPlayChime([880]); }
function ldSetMuted(muted) {
  ldState.muted = muted;
  if (_ldMasterGain && _ldAudioCtx) {
    _ldMasterGain.gain.linearRampToValueAtTime(muted ? 0 : 0.14, _ldAudioCtx.currentTime + 0.15);
  }
  const btn = $("#ldSoundBtn");
  if (btn) { btn.textContent = muted ? "🔇" : "🔊"; btn.setAttribute("aria-pressed", String(muted)); }
}
...
/* ---------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------- */
function ldResizeCanvas(forceReseed) {
  const canvas = ldState.canvas;
  const stage = $("#ldStage");
  if (!canvas || !stage) return;
  const rect = stage.getBoundingClientRect();
  const dpr = Math.min(window.devicePixelRatio || 1, 2);
  ...
  canvas.style.width = w + "px";
  canvas.style.height = h + "px";
  ldState.W = w; ldState.H = h; ldState.dpr = dpr;
  ldState.ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  if (forceReseed) ldState.particles = [];
}

function ldSpawnAmbient() {
  const mode = ldState.ambientMode;
  if (mode === "dust") {
    return { kind: "dust", x: Math.random() * ldState.W, y: ldState.H + 10,
      vx: (Math.random() - 0.5) * 5, vy: -(Math.random() * 10 + 4),
      r: Math.random() * 1.8 + 0.5, life: 0, maxLife: Math.random() * 7 + 6,
      color: [255, 224, 178] };
  }
  if (mode === "sparkle") {
    return { kind: "sparkle", x: Math.random() * ldState.W, y: Math.random() * ldState.H,
      vx: 0, vy: 0, r: Math.random() * 1.6 + 0.6, life: 0, maxLife: Math.random() * 2 + 1.4,
      color: [232, 200, 184] };
  }
  if (mode === "rain") {
    return { kind: "rain", x: Math.random() * ldState.W, y: -10,
      vx: -30, vy: Math.random() * 220 + 260, r: 1, life: 0, maxLife: 3,
      color: [190, 205, 225] };
  }
  return null;
}

function ldSpawnBurst(x, y, count, color, kind) {
  for (let i = 0; i < count; i++) {
    const a = Math.random() * Math.PI * 2;
    const speed = Math.random() * 70 + 20;
    ldState.particles.push({
      kind: kind || "burst", x, y,
      vx: Math.cos(a) * speed, vy: Math.sin(a) * speed - 30,
      r: Math.random() * 2.4 + 1, life: 0, maxLife: Math.random() * 1.3 + 0.9,
      color, drag: 0.94,
    });
  }
}

function ldStartLoop() {
  if (ldState.running) return;
  ldState.running = true;
  ldState.lastT = performance.now();
  const loop = (now) => {
    if (!ldState.running) return;
    const dt = Math.min(0.05, (now - ldState.lastT) / 1000);
    ldState.lastT = now;
    ldUpdate(dt);
    ldDraw();
    ldState.rafId = requestAnimationFrame(loop);
  };
  ldState.rafId = requestAnimationFrame(loop);
}
function ldStopLoop() {
  ldState.running = false;
  if (ldState.rafId) cancelAnimationFrame(ldState.rafId);
  ldState.rafId = null;
}

function ldUpdate(dt) {
  if (ldState.ambientMode !== "none" && (ldState.ambientCount || 0) < 60 && Math.random() < 0.5) {
    const p = ldSpawnAmbient();
    if (p) ldState.particles.push(p);
  }
  ldState.particles.forEach((p) => {
    p.life += dt;
    if (p.kind === "burst") { p.vx *= p.drag; p.vy *= p.drag; p.vy += 30 * dt; }
    p.x += p.vx * dt;
    p.y += p.vy * dt;
  });
  let liveAmbient = 0;
  ldState.particles = ldState.particles.filter((p) => {
    const alive = p.life < p.maxLife && p.y > -30 && p.y < ldState.H + 40;
    if (alive && (p.kind === "dust" || p.kind === "sparkle" || p.kind === "rain")) liveAmbient++;
    return alive;
  });
  ldState.ambientCount = liveAmbient;
}

function ldDraw() {
  const ctx = ldState.ctx;
  const { W, H } = ldState;
  if (!ctx || W === 0) return;
  ctx.clearRect(0, 0, W, H);
  ldState.particles.forEach((p) => {
    const t = p.life / p.maxLife;
    let alpha, glowMul = 4;
    if (p.kind === "sparkle") alpha = Math.sin(t * Math.PI);
    else if (p.kind === "rain") alpha = 0.5;
    else if (p.kind === "burst" || p.kind === "burst-heart" || p.kind === "burst-steam") alpha = 1 - t;
    else if (p.kind === "lantern-rise" || p.kind === "lantern-settle") { alpha = Math.min(0.9, p.life * 1.4); glowMul = 7; }
    else alpha = Math.sin(t * Math.PI) * (1 - t * 0.3);
    ...
    if (p.kind === "rain") {
      ...
      const grad = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, p.r * glowMul);
      grad.addColorStop(0, `rgba(${c[0]},${c[1]},${c[2]},0.9)`);
      grad.addColorStop(1, `rgba(${c[0]},${c[1]},${c[2]},0)`);
      ctx.fillStyle = grad;
      ctx.beginPath();
      ctx.arc(p.x, p.y, p.r * glowMul, 0, Math.PI * 2);
      ctx.fill();
    }
  });
  ctx.globalAlpha = 1;
...
/* ---------------------------------------------------------------------------
   BACKGROUND CROSSFADE + LAYER / PROGRESS HELPERS
   --------------------------------------------------------------------------- */
function ldCrossfadeBg(gradient) {
  const nextId = ldState.bgActive === "a" ? "ldBgB" : "ldBgA";
  const prevId = ldState.bgActive === "a" ? "ldBgA" : "ldBgB";
  const next = $("#" + nextId), prev = $("#" + prevId);
  if (!next || !prev) return;
  next.style.background = gradient;
  next.classList.add("ld-bg-active");
  prev.classList.remove("ld-bg-active");
  ldState.bgActive = ldState.bgActive === "a" ? "b" : "a";
}

function ldShowLayer(id) {
  const el = $("#" + id);
  if (!el) return;
  el.hidden = false;
  requestAnimationFrame(() => el.classList.add("ld-visible"));
}
function ldHideLayer(id) {
  const el = $("#" + id);
  if (!el) return;
  el.classList.remove("ld-visible");
  ldAfter(900, () => { if (el) el.hidden = true; });
}

function ldBuildProgressDots() {
  const holder = $("#ldProgress");
  if (!holder) return;
  holder.innerHTML = "";
  for (let i = LD_DOT_RANGE[0]; i <= LD_DOT_RANGE[1]; i++) {
    const dot = document.createElement("span");
    dot.className = "ld-progress-dot";
    dot.dataset.scene = String(i);
    holder.appendChild(dot);
  }
}
function ldUpdateProgressDots() {
  const holder = $("#ldProgress");
  if (!holder) return;
  const inRange = ldState.sceneIndex >= LD_DOT_RANGE[0] && ldState.sceneIndex <= LD_DOT_RANGE[1];
  holder.hidden = !inRange;
  $$(".ld-progress-dot").forEach((dot) => {
    const i = Number(dot.dataset.scene);
    dot.classList.toggle("ld-dot-active", i === ldState.sceneIndex);
    dot.classList.toggle("ld-dot-done", i < ldState.sceneIndex);
  });
}

function ldShowContinue(label, onClick) {
  const btn = $("#ldContinueBtn");
  if (!btn || ldState.continueArmed) return; // already armed for this scene — never double-wire
  ldState.continueArmed = true;
  btn.textContent = label || "Continue";
  btn.hidden = false;
  requestAnimationFrame(() => btn.classList.add("ld-visible-btn"));
  const handler = () => {
    ldState.continueArmed = false;
    btn.classList.remove("ld-visible-btn");
    ldAfter(500, () => { btn.hidden = true; });
    btn.removeEventListener("click", handler);
    onClick();
  };
  btn.addEventListener("click", handler);
}
function ldHideContinue() {
  ldState.continueArmed = false;
  const btn = $("#ldContinueBtn");
  if (!btn) return;
  btn.classList.remove("ld-visible-btn");
  ldAfter(400, () => { if (btn) btn.hidden = true; });
}

/* ---------------------------------------------------------------------------
   SCENE DISPATCH
   --------------------------------------------------------------------------- */
function ldGoToScene(index) {
  const prevId = LD_LAYER_IDS[ldState.sceneIndex];
  if (prevId && index !== ldState.sceneIndex) ldHideLayer(prevId);
  ldState.sceneIndex = index;
  ldUpdateProgressDots();
  ldCrossfadeBg(LD_BG_THEMES[index] || LD_BG_THEMES[0]);
  ldState.ambientMode = LD_AMBIENT_MODES[index] || "dust";

  const runners = [
    ldSceneIntro, ldSceneMorning, ldSceneDrive, ldSceneGrocery, ldSceneCooking,
    ldSceneRainy, ldSceneTravel, ldSceneFestivals, ldSceneStorm, ldSceneOldTogether,
    ldScenePromises, ldSceneFinale, ldSceneNotebook,
  ];
  const runner = runners[index];
  if (runner) ldAfter(prevId && index !== 0 ? 450 : 0, runner);
}
function ldAdvance() { ldGoToScene(ldState.sceneIndex + 1); }

/* ---------------------------------------------------------------------------
   SCENE 0 — INTRO
   --------------------------------------------------------------------------- */
function ldSceneIntro() {
  ldShowLayer("ldIntro");
  const textHolder = $("#ldIntroText");
  textHolder.innerHTML = "";
  const beginBtn = $("#ldBeginBtn");
  beginBtn.hidden = true;
  beginBtn.classList.remove("ld-visible-btn");
  beginBtn.removeAttribute("disabled");

  ldAfter(900, () => {
    ldTypeLd(textHolder, LD_INTRO_LINES, 32, 1100, () => {
      beginBtn.hidden = false;
      requestAnimationFrame(() => beginBtn.classList.add("ld-visible-btn"));
    });
  });
}

function ldBeginJourney() {
  if (ldState.sceneIndex !== 0) return; // guard against double-firing
  const btn = $("#ldBeginBtn");
...
/* ---------------------------------------------------------------------------
   SCENE 1 — MORNING TOGETHER
   --------------------------------------------------------------------------- */
function ldSceneMorning() {
  const holder = $("#ldMorningScene");
  holder.innerHTML = "";
  LD_MORNING_ITEMS.forEach((item, i) => {
    const span = document.createElement("span");
    span.className = "ld-morning-item";
    span.textContent = item.icon;
    span.style.left = item.x; span.style.top = item.y;
    span.style.animationDelay = `${i * 0.3}s`;
    holder.appendChild(span);
  });
  ldShowLayer("ldMorning");
  ldAfter(3200, () => ldShowContinue("Continue", ldAdvance));
}

/* ---------------------------------------------------------------------------
   SCENE 2 — WEEKEND DRIVE
   --------------------------------------------------------------------------- */
function ldSceneDrive() {
  const parallax = $("#ldDriveParallax");
  const hint = $("#ldDriveHint");
  hint?.classList.remove("ld-hint-hidden");
  ldState.driveOffset = 0;
  parallax.style.transform = "translateX(0px)";
  parallax.style.background = `
    radial-gradient(circle at 8% 85%, rgba(20,40,55,0.9) 0%, transparent 12%),
    radial-gradient(circle at 20% 90%, rgba(20,40,55,0.9) 0%, transparent 14%),
    radial-gradient(circle at 33% 82%, rgba(30,55,70,0.8) 0%, transparent 16%),
    radial-gradient(circle at 55% 88%, rgba(60,45,40,0.85) 0%, transparent 18%),
    radial-gradient(circle at 68% 84%, rgba(70,50,42,0.8) 0%, transparent 16%),
    radial-gradient(circle at 85% 90%, rgba(90,55,45,0.85) 0%, transparent 20%),
    linear-gradient(180deg, #3b4a63 0%, #6c5a52 45%, #a8664a 75%, #c98a5c 100%)
  `;
  ldShowLayer("ldDrive");
  const dialogue = $("#ldDriveDialogue");
  dialogue.textContent = "";
  ldTypeLd(dialogue, LD_DRIVE_LINES, 30, 1000, () => ldAfter(1500, () => ldShowContinue("Keep driving", ldAdvance)));
}

function ldInitDriveDrag() {
  const win = $("#ldDriveWindow");
  const parallax = $("#ldDriveParallax");
  const hint = $("#ldDriveHint");
  if (!win || !parallax) return;
  let dragging = false, startX = 0, startOffset = 0;
  const bound = 140;
  const onDown = (e) => {
    dragging = true; ldState.dragging = true;
    startX = e.touches ? e.touches[0].clientX : e.clientX;
    startOffset = ldState.driveOffset;
    win.classList.add("ld-dragging");
    hint?.classList.add("ld-hint-hidden");
  };
  const onMove = (e) => {
    if (!dragging) return;
    const x = e.touches ? e.touches[0].clientX : e.clientX;
    const next = Math.max(-bound, Math.min(bound, startOffset + (x - startX)));
    ldState.driveOffset = next;
    parallax.style.transform = `translateX(${next}px)`;
  };
  const onUp = () => { dragging = false; ldState.dragging = false; win.classList.remove("ld-dragging"); };
...
/* ---------------------------------------------------------------------------
   SCENE 3 — GROCERY SHOPPING
   --------------------------------------------------------------------------- */
function ldSceneGrocery() {
  ldState.cartCount = 0;
  $("#ldCartCount").textContent = "0";
  const grid = $("#ldGroceryGrid");
  grid.innerHTML = "";
  LD_GROCERY_ITEMS.forEach((item) => {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "ld-grocery-item";
    btn.innerHTML = `<span class="ld-grocery-icon">${item.icon}</span><span class="ld-grocery-label">${item.label}</span><span class="ld-grocery-pop">💕</span>`;
    btn.addEventListener("click", () => ldToggleGroceryItem(btn));
    grid.appendChild(btn);
  });
  ldShowLayer("ldGrocery");
  ldAfter(6500, () => ldShowContinue("Continue", ldAdvance));
}
function ldToggleGroceryItem(btn) {
  const added = btn.classList.toggle("ld-item-added");
  ldState.cartCount += added ? 1 : -1;
  if (ldState.cartCount < 0) ldState.cartCount = 0;
  $("#ldCartCount").textContent = String(ldState.cartCount);
  const cart = $("#ldCart");
  cart.classList.remove("ld-cart-bump");
  void cart.offsetWidth;
  cart.classList.add("ld-cart-bump");
  if (added) {
    ldPlayPop();
    const rect = btn.getBoundingClientRect();
    const stageRect = $("#ldStage").getBoundingClientRect();
    ldSpawnBurst(rect.left - stageRect.left + rect.width / 2, rect.top - stageRect.top + rect.height / 2, 10, [232, 180, 184], "burst-heart");
    if (ldState.cartCount === 1) ldShowContinue("Continue", ldAdvance);
  }
}

/* ---------------------------------------------------------------------------
   SCENE 4 — COOKING TOGETHER
   --------------------------------------------------------------------------- */
function ldSceneCooking() {
  ldState.cookingPicked = [];
  ldState.cookingCombosSeen = new Set();
  $("#ldCookingResult").textContent = "\u00a0";
  const holder = $("#ldCookingIngredients");
  holder.innerHTML = "";
  LD_COOKING_INGREDIENTS.forEach((item) => {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "ld-ingredient";
    btn.dataset.label = item.label;
    btn.innerHTML = `<span>${item.icon}</span><span>${item.label}</span>`;
    btn.addEventListener("click", () => ldPickIngredient(btn, item));
    holder.appendChild(btn);
  });
  ldShowLayer("ldCooking");
  ldAfter(7000, () => ldShowContinue("Continue", ldAdvance));
}
function ldPickIngredient(btn, item) {
  if (ldState.cookingPicked.length >= 2) return; // a combo is already resolving
  if (btn.classList.contains("ld-ingredient-picked")) return;
  btn.classList.add("ld-ingredient-picked");
  ldState.cookingPicked.push({ btn, item });
  if (ldState.cookingPicked.length === 2) ldResolveCombo();
}
function ldResolveCombo() {
  const [a, b] = ldState.cookingPicked;
  const key = [a.item.label, b.item.label].sort().join(",");
  const result = LD_COOKING_COMBOS[key] || LD_COOKING_FALLBACK;
  const pot = $("#ldCookingPot");
  pot.classList.remove("ld-pot-bounce");
  void pot.offsetWidth;
  pot.classList.add("ld-pot-bounce");
  $("#ldCookingResult").textContent = result;
  const potRect = pot.getBoundingClientRect();
  const stageRect = $("#ldStage").getBoundingClientRect();
  ldSpawnBurst(potRect.left - stageRect.left + potRect.width / 2, potRect.top - stageRect.top, 16, [255, 214, 170], "burst-steam");
  ldPlayPop();
  if (!ldState.cookingCombosSeen.has(key)) {
    ldState.cookingCombosSeen.add(key);
    ldShowContinue("Continue", ldAdvance);
  }
  ldAfter(1300, () => {
    a.btn.classList.remove("ld-ingredient-picked");
    b.btn.classList.remove("ld-ingredient-picked");
    ldState.cookingPicked = [];
  });
}

/* ---------------------------------------------------------------------------
   SCENE 5 — RAINY EVENING
   --------------------------------------------------------------------------- */
const LD_RAINY_LINES = ["Soft piano. Nowhere else to be.", "The room feels warmer already."];
function ldSceneRainy() {
  const btn = $("#ldHugBtn");
  btn.classList.remove("ld-glow-used");
  btn.disabled = false;
  $("#ldRainyCaption").textContent = LD_RAINY_LINES[0];
  ldShowLayer("ldRainy");
}
function ldPressHug() {
  const btn = $("#ldHugBtn");
  if (btn.disabled) return;
  btn.disabled = true;
  btn.classList.add("ld-glow-used");
  ldState.ambientMode = "lantern";
  ldPlayChime([392, 523.25, 659.25]);
  const capt = $("#ldRainyCaption");
  if (capt) capt.textContent = LD_RAINY_LINES[1];
  ldSpawnBurst(ldState.W / 2, ldState.H / 2, 26, [255, 224, 178], "burst");
  ldAfter(2200, ldAdvance);
}

/* ---------------------------------------------------------------------------
   SCENE 6 — TRAVELLING TOGETHER
   --------------------------------------------------------------------------- */
function ldSceneTravel() {
  const holder = $("#ldTravelIcons");
  holder.innerHTML = "";
  LD_TRAVEL_ICONS.forEach((item, i) => {
    const span = document.createElement("span");
    span.className = "ld-travel-icon";
    span.textContent = item.icon;
    span.style.top = item.top; span.style.left = item.left;
    span.style.animationDelay = `${i * 0.4}s`;
    holder.appendChild(span);
  });
  ldShowLayer("ldTravel");
  const dialogue = $("#ldTravelDialogue");
  dialogue.textContent = "";
  ldTypeLd(dialogue, LD_TRAVEL_LINES, 30, 1000, () => ldAfter(1400, () => ldShowContinue("Continue", ldAdvance)));
}

/* ---------------------------------------------------------------------------
   SCENE 7 — FESTIVALS
   --------------------------------------------------------------------------- */
function ldSceneFestivals() {
  const holder = $("#ldFestivalChips");
  holder.innerHTML = "";
  LD_FESTIVAL_CHIPS.forEach((chip) => {
    const div = document.createElement("div");
    div.className = "ld-festival-chip";
    div.innerHTML = `<span class="ld-festival-icon">${chip.icon}</span><span class="ld-festival-label">${chip.label}</span>`;
    holder.appendChild(div);
  });
  ldShowLayer("ldFestivals");
  ldAfter(3200, () => ldShowContinue("Continue", ldAdvance));
}

/* ---------------------------------------------------------------------------
   SCENE 8 — DIFFICULT DAYS
   --------------------------------------------------------------------------- */
function ldSceneStorm() {
  const btn = $("#ldStayBtn");
  btn.classList.remove("ld-glow-used");
  btn.disabled = false;
  $("#ldStormCaption").textContent = LD_STORM_LINES[0];
  ldShowLayer("ldStorm");
}
function ldPressStay() {
  const btn = $("#ldStayBtn");
  if (btn.disabled) return;
  btn.disabled = true;
  btn.classList.add("ld-glow-used");
  ldPlayChime([293.7, 370, 440]);
  ldCrossfadeBg(LD_BG_THEMES[8].replace("#2a2f3a", "#4a3a2a").replace("#0a0c10", "#1a140a"));
  $("#ldStormCaption").textContent = LD_STORM_LINES[1];
  ldAfter(2400, ldAdvance);
}

/* ---------------------------------------------------------------------------
   SCENE 9 — GROWING OLD TOGETHER
   --------------------------------------------------------------------------- */
const LD_YEARS_PLANT_STAGES = ["🌱", "🌿", "🪴", "🌳"];
function ldSceneOldTogether() {
  const slider = $("#ldYearsSlider");
  slider.value = "0";
  ldState.yearsValue = 0;
  ldUpdateYearsVisual(0);
  const holder = $("#ldYearsScene");
  holder.innerHTML = `
    <span class="ld-years-couple" id="ldYearsCouple">🧑‍🤝‍🧑</span>
    <span class="ld-years-plant" id="ldYearsPlant">🌱</span>
    <span class="ld-years-books" id="ldYearsBooks"></span>
  `;
  ldShowLayer("ldOldTogether");
  ldAfter(9000, () => ldShowContinue("Continue", ldAdvance));
}
function ldUpdateYearsVisual(v) {
  const label = $("#ldYearsLabel");
  const plant = $("#ldYearsPlant");
  const couple = $("#ldYearsCouple");
  const books = $("#ldYearsBooks");
  if (!label) return;
  const stage = Math.min(LD_YEARS_PLANT_STAGES.length - 1, Math.floor(v / 26));
  if (plant) plant.textContent = LD_YEARS_PLANT_STAGES[stage];
  if (couple) couple.style.filter = `grayscale(${Math.min(1, v / 100)}) sepia(${Math.min(0.5, v / 200)})`;
  if (books) books.textContent = "📚".repeat(1 + Math.floor(v / 18));
  const years = Math.round(v / 2); // slider 0-100 -> roughly half a century
  label.textContent = years <= 1 ? "Year One" : `${years} Years Together`;
}
function ldInitYearsSlider() {
  const slider = $("#ldYearsSlider");
  if (!slider) return;
  slider.addEventListener("input", () => {
    const v = Number(slider.value);
    ldState.yearsValue = v;
    ldUpdateYearsVisual(v);
    if (v >= 96 && !slider.dataset.doneShown) {
      slider.dataset.doneShown = "1";
      ldShowContinue("Continue", ldAdvance);
    }
  });
}
...
/* ---------------------------------------------------------------------------
   SCENE 10 — PROMISE WALL
   --------------------------------------------------------------------------- */
function ldShuffledPromises() {
  const arr = LD_PROMISES.map((text, i) => ({ text, i }));
...
function ldScenePromises() {
  ldState.promisesOrder = ldShuffledPromises();
  ldState.promisesRevealed = 0;
  $("#ldPromiseCount").textContent = "0 of 100 promises found";
  const wall = $("#ldPromiseWall");
  wall.innerHTML = "";
  ldState.promisesOrder.forEach((entry, idx) => {
    const tile = document.createElement("button");
    tile.type = "button";
    tile.className = "ld-promise-tile";
    tile.innerHTML = `
      <span class="ld-promise-tile-inner">
        <span class="ld-promise-face ld-promise-front">🤍</span>
        <span class="ld-promise-face ld-promise-back">${entry.text}</span>
      </span>`;
    tile.addEventListener("click", () => ldRevealPromise(tile), { once: true });
    wall.appendChild(tile);
  });
  ldShowLayer("ldPromises");
  ldAfter(4000, () => ldShowContinue("Continue", ldAdvance));
}
function ldRevealPromise(tile) {
  tile.classList.add("ld-tile-revealed");
  ldState.promisesRevealed++;
  $("#ldPromiseCount").textContent = `${ldState.promisesRevealed} of 100 promises found`;
  ldPlayPop();
}

/* ---------------------------------------------------------------------------
   SCENE 11 — FINAL LANTERNS
   --------------------------------------------------------------------------- */
function ldHeartPoint(t, cx, cy, scale) {
  // Classic parametric heart curve, t in [0, 2π)
  const x = 16 * Math.pow(Math.sin(t), 3);
  const y = 13 * Math.cos(t) - 5 * Math.cos(2 * t) - 2 * Math.cos(3 * t) - Math.cos(4 * t);
  return { x: cx + x * scale, y: cy - y * scale };
}
function ldSceneFinale() {
  ldShowLayer("ldFinale");
  const textHolder = $("#ldFinaleText");
  textHolder.innerHTML = "";
  ldSpawnLanternField();
  ldAfter(700, () => {
    ldTypeLd(textHolder, LD_FINALE_LINES, 32, 1300, () => {
      ldAfter(1600, ldFormHeartConstellation);
    });
  });
}
function ldSpawnLanternField() {
  const count = 46;
  for (let i = 0; i < count; i++) {
    ldState.particles.push({
      kind: "lantern-rise", x: Math.random() * ldState.W, y: ldState.H + Math.random() * 200,
      vx: (Math.random() - 0.5) * 6, vy: -(Math.random() * 12 + 10),
      r: Math.random() * 1.6 + 1.6, life: 0, maxLife: 999,
      color: [255, 200, 140], settled: false, targetT: (i / count) * Math.PI * 2,
    });
  }
}
function ldFormHeartConstellation() {
  const myGen = ldState.gen;
  const cx = ldState.W / 2, cy = ldState.H * 0.42, scale = Math.min(ldState.W, ldState.H) / 34;
  ldState.particles.forEach((p) => {
    if (p.kind !== "lantern-rise") return;
    const target = ldHeartPoint(p.targetT, cx, cy, scale);
    p.targetX = target.x; p.targetY = target.y;
    p.kind = "lantern-settle";
    p.vx = 0; p.vy = 0; // stop the generic per-frame integration from fighting the lerp below
  });
  const start = performance.now();
  const step = (now) => {
    if (ldState.gen !== myGen) return;
    const t = Math.min(1, (now - start) / 2400);
    ldState.particles.forEach((p) => {
      if (p.kind !== "lantern-settle" || p.targetX == null) return;
      p.x = p.x + (p.targetX - p.x) * 0.06;
      p.y = p.y + (p.targetY - p.y) * 0.06;
    });
    if (t < 1) requestAnimationFrame(step);
    else ldAfter(1800, () => ldGoToScene(12));
  };
  requestAnimationFrame(step);
}

/* ---------------------------------------------------------------------------
   SECRET ENDING — THE NOTEBOOK
   --------------------------------------------------------------------------- */
const LD_NOTE_KEY = "radhika_next_memory_note";
function ldSceneNotebook() {
  const fade = $("#ldFade");
  fade?.classList.add("ld-fade-on");
  ldAfter(900, () => {
    ldShowLayer("ldNotebook");
    ldAfter(500, () => fade?.classList.remove("ld-fade-on"));
    const saved = localStorage.getItem(LD_NOTE_KEY);
    const input = $("#ldNotebookInput");
    const savedLabel = $("#ldNotebookSaved");
    if (saved) { input.value = saved; savedLabel.hidden = false; }
    else savedLabel.hidden = true;
  });
}
function ldSaveNote() {
  const input = $("#ldNotebookInput");
  ...
  if (!value) return;
  try { localStorage.setItem(LD_NOTE_KEY, value); } catch (e) { /* storage unavailable — fail quietly */ }
  savedLabel.hidden = false;
  ldPlayChime([523.25, 659.25, 783.99]);
}

/* ---------------------------------------------------------------------------
   TINY REUSABLE TYPEWRITER (module-local, same shape as tmTypeLines)
   --------------------------------------------------------------------------- */
function ldTypeLd(el, lines, charDelay, linePause, onDone) {
  el.innerHTML = "";
  let i = 0;
  let cancelled = false;
  function nextLine() {
    ...
    const timer = setInterval(() => {
      ...
      if (c >= text.length) {
        clearInterval(timer);
        i++;
        setTimeout(() => { if (!cancelled) nextLine(); }, linePause);
      }
    }, charDelay);
  }
  nextLine();
  const cancelFn = () => { cancelled = true; };
  ldState.cancelFns.push(cancelFn);
}

/* ---------------------------------------------------------------------------
   RESET / STOP
   --------------------------------------------------------------------------- */
function resetLifeDreamFlow() {
  ldState.gen++;
  ldClearTimers();
  ldStopPad();
  ldState.sceneIndex = 0;
  ldState.particles = [];
  ldState.ambientCount = 0;
  ldState.ambientMode = "dust";
  ldState.cartCount = 0;
  ldState.cookingPicked = [];
  ldState.driveOffset = 0;
  ldState.yearsValue = 0;
  ...
  LD_LAYER_IDS.forEach((id) => {
    const el = $("#" + id);
    if (!el) return;
    el.hidden = true;
    el.classList.remove("ld-visible");
  });
  $("#ldFade")?.classList.remove("ld-fade-on");
  ldHideContinue();
  const yearsSlider = $("#ldYearsSlider");
  if (yearsSlider) { yearsSlider.value = "0"; delete yearsSlider.dataset.doneShown; }
  const hugBtn = $("#ldHugBtn");
  if (hugBtn) { hugBtn.disabled = false; hugBtn.classList.remove("ld-glow-used"); }
  const stayBtn = $("#ldStayBtn");
  if (stayBtn) { stayBtn.disabled = false; stayBtn.classList.remove("ld-glow-used"); }

  if (!ldState.canvas) return;
  ldResizeCanvas(true);
  ldBuildProgressDots();
  ldStartLoop();
  ldGoToScene(0);
...
/* ---------------------------------------------------------------------------
   INIT
   --------------------------------------------------------------------------- */
function initLifeDreamPage() {
  const canvas = $("#ldCanvas");
  if (!canvas || ldState.canvas) return;
  ldState.canvas = canvas;
  ldState.ctx = canvas.getContext("2d");

  let resizeRaf = null;
  window.addEventListener("resize", () => {
    if ($("#view-lifedream").hidden) return;
    if (resizeRaf) cancelAnimationFrame(resizeRaf);
    resizeRaf = requestAnimationFrame(() => ldResizeCanvas());
  });

  $("#ldBeginBtn")?.addEventListener("click", ldBeginJourney);
  ldInitDriveDrag();
  ldInitYearsSlider();
  $("#ldHugBtn")?.addEventListener("click", ldPressHug);
  $("#ldStayBtn")?.addEventListener("click", ldPressStay);
  $("#ldNotebookSaveBtn")?.addEventListener("click", ldSaveNote);
  $("#ldSoundBtn")?.addEventListener("click", () => ldSetMuted(!ldState.muted));
}
